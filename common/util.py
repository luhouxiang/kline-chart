...
try:
    ...
    # Fallback so logging still works without the optional dependency.
...
class SysLogInit(object):
    def __init__(self, name: str, path: str = '', type: int = 0):
        ...
        handler = ConcurrentRotatingFileHandler(all_error_log_path, 'a', 5242880, 10)
        ...
        if 0 == type:
            log_handlers.append(ConcurrentRotatingFileHandler(file_path, 'a', 5242880, 10))
        elif 1 == type:
